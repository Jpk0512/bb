...
function providerProcessEnvFromShellEnv(
  ...
  // removes BB_* variables. These values are the stable daemon/CLI routing
  // context that provider-launched `bb` commands need. Thread-scoped values
  // (BB_THREAD_ID, BB_THREAD_STORAGE, etc.) are supplied per request through
  // the adapter envVars and must not be made runtime-global: one environment
  // can host multiple threads concurrently.
  for (const key of [
    "BB_CLI",
    "BB_SERVER_URL",
    "BB_HOST_DAEMON_PORT",
