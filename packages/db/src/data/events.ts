...
export function insertEvents(
  ...
  for (const [index, input] of eventInputs.entries()) {
    ...
    const result = db.run(
      sql`INSERT OR IGNORE INTO events (id, thread_id, environment_id, scope_kind, turn_id, provider_thread_id, sequence, type, item_id, item_kind, parent_tool_call_id, data, created_at, daemon_event_id)
          VALUES (${id}, ${input.threadId}, ${input.environmentId ?? null}, ${input.scope.kind}, ${turnId}, ${input.providerThreadId ?? null}, ${input.sequence}, ${input.type}, ${input.itemId}, ${input.itemKind}, ${input.parentToolCallId}, ${input.data}, ${createdAt}, ${null})`,
    );
    if (result.changes > 0) {
      insertedCount++;
  ...
  for (const [index, input] of eventInputs.entries()) {
    ...
    db.run(
      sql`INSERT INTO events
        (id, thread_id, environment_id, scope_kind, turn_id, provider_thread_id, sequence, type, item_id, item_kind, parent_tool_call_id, data, created_at, daemon_event_id)
        VALUES (
          ${createEventId()},
          ${input.threadId},
          ${input.environmentId},
...
export function copyStoredThreadEventsInTransaction(
  ...
  for (const row of args.rows) {
    ...
    const event = parseDaemonThreadEvent({
      daemonEventId: row.daemonEventId ?? createEventId(),
      data: row.data,
...
// --- fork-specific event helpers (retained during 0.40.0 merge) ---
